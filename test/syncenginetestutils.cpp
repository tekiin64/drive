    ...
    : FakeReply { parent }
{
    ...
    auto writeFileResponse = [&](const FileInfo &fileInfo) {
        ...
        auto currentPermissions = !fileInfo.permissions.isNull() ? QString(fileInfo.permissions.toString()) : fileInfo.isShared ? QStringLiteral("SRDNVCKW") : QStringLiteral("RDNVCKW");
        if (fileInfo.isMountedRoot) {
            currentPermissions += "M";
        } else if (fileInfo.isMounted) {
            currentPermissions += "m";
        }
        xml.writeTextElement(ocUri, QStringLiteral("permissions"), currentPermissions);
